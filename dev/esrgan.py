...
class VGGStyleDiscriminator(nn.Module):
    ...
    def forward(self, x):
        assert x.size(2) == self.input_size, f'Input size must be identical to input_size, but got {x.size()} and {self.input_size}'
...
def load_model_and_config(model_path: Path, checkpoint='latest'):
    """Load ESRGAN model from checkpoint with given network configuration."""
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model_path = Path(model_path)

    # Find the config file (could be .yml or .yaml)
    config_files = list(model_path.glob('*.yml')) + list(model_path.glob('*.yaml'))
    if not config_files:
    ...
    if checkpoint == 'latest':
        checkpoint_path = model_path / "models" / 'net_g_latest.pth'
    else:
        checkpoint_path = model_path / "models" / f'net_g_{checkpoint}.pth'

    if not checkpoint_path.exists():
        # Try to find the latest checkpoint
        available_checkpoints = sorted((model_path / "models").glob('net_g_*.pth'))
        if available_checkpoints:
            checkpoint_path = available_checkpoints[-1]
            print(f"Using checkpoint: {checkpoint_path.name}")
        else:
            raise FileNotFoundError(f"No checkpoint found in {model_path / 'models'}")

    config = yaml.load(config_path.read_text(), Loader=yaml.FullLoader)
    network_config = config['network_g']
    network_config.pop('type')  # Remove the type field

    model = RRDBNet(**network_config)
